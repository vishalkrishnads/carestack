    ...
    pub async fn search(&self, request: Value) -> HttpResponse {
        ...
        match serde_json::from_value::<Value>(request["query"].clone()) {
            Ok(query) => {
                ...
                let filter = doc! {
                    "$or": [
                        { "username": { "$regex": &query.to_string().trim_matches('"'), "$options": "i" } },
                        { "name": { "$regex": &query.to_string().trim_matches('"'), "$options": "i" } }
                    ]
                };
                    ...
                    while flag {
                        if let Ok(remains) = cursor.advance().await {
                            if !remains { flag = false; }
                                ...
                                if let Ok(friend_obj) = cursor.deserialize_current() {users.push(friend_obj)}
                            }
                        }
                    }
                }

                HttpResponse::Ok().body(json!({"results": users}).to_string())
            },
